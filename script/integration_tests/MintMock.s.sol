...
/// @notice Mints mock prediction market ERC-1155 tokens (YES and NO) to a recipient wallet.
///         Used to fund a test wallet before running BridgeTokens.s.sol.
///
/// ─── Required env vars ───────────────────────────────────────────────────────
///
///   DEPLOYER_PRIVATE_KEY   Wallet paying gas (must be able to call mint on MockPredictionMarket)
///   PREDICTION_MARKET_CONTRACT       MockPrediction market contract address on BSC testnet
///   OWNER_ADDRESS          Recipient of the minted tokens
///
/// ─── Notes ───────────────────────────────────────────────────────────────────
///
///   - TOKEN_ID_YES and TOKEN_ID_NO must match the IDs used in BridgeTokens.s.sol
///     and registered in PredictionMarketEscrow.
...
contract MintMock is Script {
    ...
    function run() external {
        ...
        IMockPredictionMarket(mock).mint(recipient, TOKEN_ID_YES, AMOUNT);
        IMockPredictionMarket(mock).mint(recipient, TOKEN_ID_NO, AMOUNT);
        vm.stopBroadcast();

        // ─── Post-mint balance check (view, no gas) ───────────────────────────

        uint256 balYes = IMockPredictionMarket(mock).balanceOf(recipient, TOKEN_ID_YES);
        uint256 balNo  = IMockPredictionMarket(mock).balanceOf(recipient, TOKEN_ID_NO);
