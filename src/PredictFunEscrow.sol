...
/// @title PredictFunEscrow
/// @notice Deployed on BSC. Locks PredictFun ERC-1155 shares in escrow and sends a LayerZero
///         message to BridgeReceiver on Polygon, which mints WrappedPredictFunToken.
///         Receives unlock messages from Polygon to release shares back to users.
/// @dev Handles a single PredictFun ERC-1155 contract (set as immutable).
///      If PredictFun deploys a new contract, deploy a new PredictFunEscrow.
///
/// ─── Deployment checklist ────────────────────────────────────────────────────
///
///   1. Deploy PredictFunEscrow (contract starts paused)
...
///
contract PredictFunEscrow is OApp, OAppOptionsType3, IERC1155Receiver, ReentrancyGuard, Pausable {
    ...
    /// @notice The PredictFun ERC-1155 contract this escrow handles.
    /// @dev Immutable — if PredictFun deploys a new contract, deploy a new PredictFunEscrow.
    address public immutable predictFunContract;
    ...
    /// @param tokenId          PredictFun ERC-1155 token ID locked.
    ...
    /// @param tokenId PredictFun ERC-1155 token ID unlocked.
    ...
    /// @param _predictFunContract PredictFun ERC-1155 contract address on BSC.
    /// @param _polygonEid     LayerZero endpoint ID for Polygon (mainnet: 30109).
    constructor(
        address _endpoint,
        address _owner,
        address _predictFunContract,
        uint32 _polygonEid
    ) OApp(_endpoint, _owner) Ownable(_owner) {
        if (_predictFunContract == address(0)) revert ZeroAddress();
        predictFunContract = _predictFunContract;
    ...
    /// @notice Lock PredictFun ERC-1155 shares in escrow and bridge them to Polygon.
    /// @dev    Caller must approve this contract on the PredictFun ERC-1155 contract first.
    ...
    /// @param _tokenId          PredictFun ERC-1155 token ID to lock.
    ...
    function lock(
        ...
        IERC1155(predictFunContract).safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");
    ...
    /// @param _tokenId          PredictFun ERC-1155 token ID.
    ...
    ///         locked PredictFun tokens to the specified BSC recipient.
    ...
    function _lzReceive(
        ...
        IERC1155(predictFunContract).safeTransferFrom(address(this), bscRecipient, tokenId, amount, "");
    ...
    /// @dev    Cannot rescue PredictFun tokens for a tokenId that still has locked balance —
    ...
    function rescueTokens(
        ...
        if (_token == predictFunContract && totalLocked[_tokenId] > 0)
