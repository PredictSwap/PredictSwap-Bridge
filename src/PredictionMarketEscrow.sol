...
/// @notice Deployed on BSC. Locks prediction market ERC-1155 shares in escrow and sends a
...
/// @dev Handles a single prediction market ERC-1155 contract (set as immutable).
///      If the prediction market deploys a new contract, deploy a new PredictionMarketEscrow.
///
/// ─── Deployment checklist ────────────────────────────────────────────────────
///
///   1. Deploy PredictionMarketEscrow (contract starts paused)
...
///
contract PredictionMarketEscrow is OApp, OAppOptionsType3, IERC1155Receiver, ReentrancyGuard, Pausable {
    ...
    /// @notice The prediction market ERC-1155 contract this escrow handles.
    /// @dev Immutable — if the prediction market deploys a new contract, deploy a new PredictionMarketEscrow.
    ...
    /// @param tokenId          Prediction market ERC-1155 token ID locked.
    ...
    /// @param tokenId Prediction market ERC-1155 token ID unlocked.
    ...
    /// @param _predictionMarketContract Prediction market ERC-1155 contract address on BSC.
    ...
    constructor(
        ...
        if (_predictionMarketContract == address(0)) revert ZeroAddress();
    ...
    /// @notice Lock prediction market ERC-1155 shares in escrow and bridge them to Polygon.
    /// @dev    Caller must approve this contract on the prediction market ERC-1155 contract first.
    ...
    /// @param _tokenId          Prediction market ERC-1155 token ID to lock.
    ...
    function lock(
        ...
        IERC1155(predictionMarketContract).safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");
    ...
    /// @param _tokenId          Prediction market ERC-1155 token ID.
    ...
    ///         locked prediction market tokens to the specified BSC recipient.
    ...
    function _lzReceive(
        ...
        IERC1155(predictionMarketContract).safeTransferFrom(address(this), bscRecipient, tokenId, amount, "");
    ...
    /// @dev    Cannot rescue prediction market tokens for a tokenId that still has locked
    ///         balance — those belong to users awaiting unlock. Other ERC-1155 tokens
    ///         (e.g. wrong token sent by mistake) can always be rescued.
    ...
    function rescueTokens(
        ...
        if (_token == predictionMarketContract && totalLocked[_tokenId] > 0)
