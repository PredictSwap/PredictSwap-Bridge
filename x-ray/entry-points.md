...
> Opinion NFT Bridge | 16 entry points | 2 permissionless | 2 role-gated | 12 admin-only
...
| OpinionEscrow | `setDstGasLimit(uint128)` | _gasLimit | dstGasLimit, enforcedOptions[polygonEid][SEND] |
...
| BridgeReceiver | `setDstGasLimit(uint128)` | _gasLimit | dstGasLimit, enforcedOptions[bscEid][SEND] |
...
### Inherited from OApp (onlyOwner)

| Contract | Function | Parameters | State Modified |
|----------|----------|------------|----------------|
| OpinionEscrow, BridgeReceiver | `setPeer(uint32,bytes32)` | _eid, _peer | peers[_eid] = _peer |
